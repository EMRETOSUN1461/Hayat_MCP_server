...
6. **Mesaj kontrolü:** Aynı mesaj zaten varsa yenisi oluşturulmaz. Bir modülün mesajı başka modülün mesaj class'ından verilmez. **MCP istisnası:** MCP message class (SE91) üretemediği için agent mesajları **hardcode olarak yazar** ve geliştirmeyi durdurmaz. KURAL #7 (S4D standart sapma kontrolü) tetiklenmez. Hardcode mesaj listesi final raporda paylaşılır; ileride manuel SE91'den `Z<XX>` message class'a migrate edilebilir.
...
Agent bir nesne oluşturma veya geliştirme talebi aldığında, **önce geliştirme tipini anlar**, ardından bilgileri talep metninden ve sistemden tespit eder. Sadece gerçekten çıkarılamayan bilgiler kullanıcıya sorulur.

### B1. Maksimum Otonom Davranış

1. **Modül (`XX`):** Talep metninden veya hedef paket adından çıkarılır (`ZHR` → HR, `ZMM` → MM, `ZFI` → FI, `ZSD` → SD, `ZPP` → PP, ...). Talepte yoksa **sorulur**.
2. **Ana paket ve alt paket — otonom seçim:**
   - Kullanıcı mevcut bir paket adı vermişse veya geliştirme zaten bir paket altında devam ediyorsa **sorulmaz**; mevcut paket kullanılır.
   - **Paket verilmemiş ama modül kısaltması var/çıkarılabiliyorsa**: agent `Z<XX>_NNN` formatındaki mevcut alt paketleri tarar (`SearchObject` veya `GetPackageContents("Z<XX>", include_subpackages=true)`); en yüksek `NNN` + 1 ile yeni paket adını otomatik belirler ve `CreatePackage` ile oluşturur (üst paket = `Z<XX>`). Yeni paket olduğu için içindeki obje numaraları 01/001'den başlar. Karar bilgi olarak iletilir.
   - Hedef paket sistemde yoksa `CreatePackage` ile oluşturulur. **Default'lar otomatik kullanılır**: Transport Layer (`ZS4D`), Software Component (`HOME`), Record Changes (`true`) — sorulmaz.
   - Modül de talepten çıkarılamıyorsa **o zaman** modül + paket sorulur.
3. **Oluşturulacak her standart nesnenin numarası — `GetPackageContents` ile tespit edilir, sorulmaz:**
   - Program: `ZXX_NNN_PNN` → mevcut paketteki en yüksek `_P<NN>` + 1
   - Include: `ZXX_NNN_PNN_INN` → en yüksek `_I<NN>` + 1
   - Class: `ZXX_NNN_CLNN` → en yüksek `_CL<NN>` + 1
   - Interface: `ZXX_NNN_IFNN` → en yüksek `_IF<NN>` + 1
   - Function Group: `ZXX_NNN_FGNN` → en yüksek `_FG<NN>` + 1
   - Function Module: `ZXX_NNN_FGNN_FMNN` → en yüksek `_FM<NN>` + 1
   - Table: `ZXX_NNN_TNN` → en yüksek `_T<NN>` + 1
   - Structure: `ZXX_NNN_SNN` → en yüksek `_S<NN>` + 1
   - Data Element: `ZXX_NNN_ENN` → en yüksek `_E<NN>` + 1
   - Domain: `ZXX_NNN_DNN` → en yüksek `_D<NN>` + 1
   - Table Type: `ZXX_NNN_TTNN` → en yüksek `_TT<NN>` + 1
   - CDS View: `ZXX_NNN_CDS_VNN` → en yüksek `_V<NN>` + 1
   - Paket boşsa veya o tipte hiç obje yoksa → 01'den başlanır. Karar bilgi olarak iletilir, onay beklenmez.
   - **İstisna — ZBCENH bağlamı:** Enhancement / BAdI / Function Exit / Screen Exit gibi ZBCENH üzerinden takip edilen geliştirmelerde tüm exit numaraları (ENHA_IMNN, ENHA_ENNN, BADI_ENNN, EXIT_ENNN, ENHA_SNN vb.) talepte geçmiyorsa kullanıcıdan istenir — sistem listesi yetersizdir, developer ZBCENH'ten merkezi numara alır.
4. **Transport request:** Agent otonom yönetir — `ListTransports` ile kullanıcının açık Workbench Request'leri kontrol edilir; uygunsa kullanılır. Yoksa `CreateTransport` ile yeni oluşturulur (açıklama formatı modül-tabanlı). Karar bilgi olarak iletilir, sorulmaz. Kullanıcı belirli bir TR adı verirse o kullanılır. `$TMP` paketi için transport gerekmez.
...
3. **Standart obje numaralarını sistemden tespit eder, sormaz.** Hedef paket biliniyorsa (talepte verilmiş veya bağlam devam ediyorsa) sıradaki standart obje numarası `GetPackageContents` ile bulunur: aynı tipteki en yüksek numara + 1; paket boşsa 01/001'den başlar. Standart objeler: P01/P02 (program), I01/I02 (include), CL01/CL02 (class), FG01/FG02 (function group), FM01/FM02 (function module), T01/T02 (tablo), S01/S02 (structure), D01/D02 (domain), E01/E02 (data element), TT01/TT02 (table type), V01/V02 (view) vb. Karar bilgi olarak iletilir, onay beklenmez. **İstisna:** ZBCENH üzerinden developer tarafından merkezi takip edilen enhancement numaraları (ENHA_IMNN, ENHA_ENNN, BADI_ENNN, EXIT_ENNN vb.) talepte geçmiyorsa **kullanıcıdan istenir** — sistem listesi bu durumda yetersizdir.
4. **Sıradaki numara için sisteme sorgu atılır.** `GetPackageContents` çağrısı zorunludur — sıradaki standart obje numarasını bulmak ve aynı paketteki çakışmaları önlemek için. Tahmin/varsayım yerine sistem gerçeği esas alınır.
5. **Nesne oluşturmadan önce tüm bilgileri toplar.** Eksik bilgiyle nesne oluşturmaya başlamaz. Talepten/sistemden çıkarılabilen bilgiler için soru sorulmaz; sadece gerçekten çıkarılamayan bilgiler (ör: structure/tablo alan listesi, data element, ZBCENH numarası) tek birleşik soruyla istenir.
...
7. **Birden fazla nesne oluşturulacaksa** (ör: exit class + include + ZBCENH kaydı), tüm nesne listesini ve sistemden tespit edilen numaraları kullanıcıya **bildirir**, onay beklemez: _"Şu objeler oluşturulacak: ZSD_999_P01, ZSD_999_I01, ZSD_999_CL01. Devam ediyorum."_ Kullanıcı itiraz ederse o noktada düzeltilir; itiraz yoksa kesintisiz devam edilir. Talepte yer almayan bilgiler (ZBCENH numaraları, alan listesi vb.) tek birleşik soruyla istenir.
...
9. **Transport numarasını otonom yönetir.** `ListTransports` ile kullanıcının açık Workbench Request'leri kontrol edilir; uygun bir tanesi varsa kullanılır. Yoksa `CreateTransport` ile yeni request oluşturulur (açıklama formatı modül-tabanlı). Karar bilgi olarak iletilir ("Yeni request `S4DK912678` oluşturuldu"), akış durmaz. Kullanıcı belirli bir transport adı verirse o değer kullanılır. (`$TMP` lokal paket için transport gerekmez.)
10. **Paket kullanımı (otonom seçim):**
    - Kullanıcı mevcut bir paket adı vermişse veya geliştirme zaten bir paket bağlamında devam ediyorsa **sorulmaz**; o paket kullanılır.
    - **Paket verilmemiş ama modül kısaltması (`XX`) var/çıkarılabiliyorsa**: agent `Z<XX>_NNN` formatındaki mevcut alt paketleri sistemden tarar (`SearchObject` veya `GetPackageContents("Z<XX>", include_subpackages=true)`); en yüksek `NNN` + 1 ile yeni paket adını otomatik belirler ve `CreatePackage` ile oluşturur. Üst paket = `Z<XX>` (ZMM, ZSD, ZFI vb.). Yeni paket olduğu için içindeki obje numaraları 01/001'den başlar — `GetPackageContents` çağrısına gerek yok. Karar bilgi olarak iletilir, sorulmaz: _"ZMM_998 sona en yüksek paketti, yeni geliştirme `ZMM_999` paketi altında oluşturulacak. Devam ediyorum."_
    - Modül kısaltması da çıkarılamıyorsa (talepte paket yok, modül de yok) sadece **o zaman** kullanıcıya sorulur.
    - Hedef paket sistemde yoksa `CreatePackage` ile oluşturulur ve aktive edilir.
    - **Default değerler her zaman otomatik:** Transport Layer (`ZS4D`), Software Component (`HOME`), Record Changes (`true`). Sorulmaz.
11. **Structure alan bilgileri — talepten alınır, yoksa sorulur.** Agent kendi başına alan listesi üretmez. **Talepte (cümle, tablo, .docx, .xlsx, JSON, vb.) `Alan Adı | Data Element | Curr/Quan Ref` formatında bilgi varsa o değerler doğrudan kullanılır, sorulmaz.** Hiç bahsi yoksa B4 bölümündeki formatta tek birleşik soruyla istenir. CURR/QUAN alanları için `Curr/Quan Ref` zorunludur (`TABLO-ALAN` formatı).
12. **Tablo alan ve özellikleri — talepten alınır, yoksa sorulur.** **Talepte `Alan Adı | Data Element | Key | Curr/Quan Ref` bilgisi varsa kullanılır, sorulmaz.** Hiç bahsi yoksa B5 bölümündeki formatta tek birleşik soruyla istenir. Search help ve SM30 bakım ekranı DDL'de desteklenmez — atlanır, sonda bildirilir.
13. **Structure ve tablolarda generic tip kullanmaz; alanları verilen isim ve sırayla oluşturur.** `abap.char(N)`, `abap.quan(N,M)`, `abap.curr(N,M)`, `abap.numc(N)`, `abap.int4` gibi built-in/generic tipler kesinlikle kullanılmaz. Her alan için mutlaka sistemdeki bir data element kullanılır (ör: `KWMENG`, `NETWR_AP`, `VBELN_VA`). Data element talepte verilmemişse ve standart bir karşılığı tahmin edilemiyorsa kullanıcıdan istenir. Alanlar kullanıcının verdiği isimle ve verdiği sırayla oluşturulur; agent hiçbir alanı yeniden adlandıramaz, sırasını değiştiremez veya kendi başına ekstra alan ekleyemez.
